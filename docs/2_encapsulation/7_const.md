...
!!! example "四种参数传递方式的对比"
    ...
    class LargeObject {
    public:
        LargeObject() : name("largeObject") {
            cout << "Constructed" << endl; 
        }
        LargeObject(const LargeObject& other) : name(other.name){
            cout << "Copied" << endl;
        }
        ~LargeObject() { cout << "Destroyed" << endl; }

        void show() const { cout << "LargeObject" << endl; }

        void modify() { name = "newName"; }
    ...
    // 方式一：传值（复制一次）
    void byValue(LargeObject obj) {
        obj.show();
    }

    // 方式二：传指针（不复制，但可能为空）
    void byPtr(const LargeObject* obj) {
        if (obj) obj->show();
    }

    // 方式三：传引用（不复制，不能为空）
    void byRef(const LargeObject& obj) {
        obj.show();
    }

    // 方式四：传可修改引用（允许修改）
    void byMutableRef(LargeObject& obj) {
        obj.modify();   // 可以修改
    }
...
!!! summary "参数传递方式选择指南"
    ...
    | :------------- | :--------------------- | :----------: | :------------: | :---------: | :----------------------- |
    | **传值**       | `void f(T param)`      |  高（复制）  | 否（局部副本） |     N/A     | 小对象，需要副本         |
    | **传指针**     | `void f(const T* p)`   | 低（不复制） |  否（const）   |     是      | 可能为空，C风格          |
    | **常引用**     | `void f(const T& ref)` | 低（不复制） |       否       |     否      | **读取大型对象（推荐）** |
    | **可修改引用** | `void f(T& ref)`       | 低（不复制） |       是       |     否      | 需要修改对象             |

!!! tip "最佳实践"

    - **优先使用常引用**（`const T&`）传递只读的大型对象：既高效又安全。
