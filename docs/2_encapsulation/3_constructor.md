...
当类被用作基类时，应该将析构函数声明为 `virtual`，以确保通过基类指针删除派生类对象时，能正确调用派生类的析构函数。
...
!!! example "虚析构函数"
    ...
    Base* p = new Derived();
    delete p;   // 正确调用 Derived 的析构函数，再调用 Base 的析构函数
    ```
...
!!! danger "非虚析构函数导致的问题"
    ...
    Base* p = new Derived();
    delete p;   // 只调用 Base 的析构函数，不调用 Derived 的析构函数 → 资源泄漏！
    ```
...
!!! summary "析构函数要点回顾"
    ...
    | **虚析构函数**     | 用户定义（加 `virtual`） | 确保派生类正确析构         | 类被设计为基类，用于多态删除     |
