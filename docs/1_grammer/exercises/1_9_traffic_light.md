...
5. 必须使用 `switch` 语句完成判断。
6. 只有 `TrafficLight::Green` 返回 `true`。
7. `TrafficLight::Red` 和 `TrafficLight::Yellow` 均返回 `false`。
8. 不得将枚举值转换为整数后再进行比较。
9. 在 `test` 函数中使用 `assert` 验证三种信号的结果。
